			...
			then
				...
				for x = 0, math.ceil(math.random(3)) do
					...
					if IsAHuman(actor[x]) then
						actor[x]:AddInventoryItem(RandomHDFirearm("Weapons - Primary", self.CPUTechName))
						actor[x]:AddInventoryItem(RandomHDFirearm("Weapons - Secondary", self.CPUTechName))
						if PosRand() < 0.5 then
							actor[x]:AddInventoryItem(RandomHDFirearm("Tools - Diggers", self.CPUTechName))
						end
					end
